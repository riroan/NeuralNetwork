...
double gradient_descent_Adam(double(*f)(double, double), double init_x, double init_y)
{
	...
	for (int i = 0; i < EPOCH; i++)
	{
		...
		double ds_x = s_x / (1 - pow(p1, t));
		double ds_y = s_y / (1 - pow(p1, t));

		double dr_x = r_x / (1 - pow(p2, t));
		double dr_y = r_y / (1 - pow(p2, t));

		double dx = -0.001*ds_x / (sqrt(dr_x) + 1e-8);
		double dy = -0.001*ds_y / (sqrt(dr_y) + 1e-8);
...
void f(int number)
{
	...
	{
		x[i] = distribution(generator)/2.0;
		mean += x[i];
		if (x[i] >= 0.0&&x[i] < 0.1)
			cnt[0]++;
		else if (x[i] < 0.2)
